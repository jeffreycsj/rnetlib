...
- 用户态 UDP 故障代理会在 UDP/KCP 的真实 socket 路径丢弃一个服务端 wire-v4 协商数据报，验证重传后仍只产生一次就绪；KCP 还会丢弃一个客户端业务数据报并验证可靠重传。该测试连续重复运行 10 次通过，但不替代宽参数弱网实验。
...
| # | 严重度 | 位置 | 问题描述 | 修复结果 |
|---|---|---|---|---|
| 1 | 高 | wire-v4 `SessionOpened` | 新恢复连接只完成传输授权、尚未完成 SELECT/ACK/READY 时就关闭旧会话；协商失败会同时失去新旧连接。 | v3 保持传输打开即接管；v4 将旧句柄撤销移动到 `publish_range_ready`。真实 UDP 测试阻断最终协商并验证超时后旧会话仍可双向收发。 |
| 2 | 高 | completed 积压下的 `poll` | 已完成的早期业务事件直接占满返回容量，底层认证 ACK 无法被消费，可能触发本地误超时。 | 返回积压事件前有界、非阻塞消化内部控制；容量 1 的真实 TCP 心跳回归持续越过超时窗口且无误关闭。 |
| 3 | 中 | 零容量游戏 poll | Rust 与 C 的零容量调用不推进任何维护状态，心跳、时钟和 v4 重试均停滞。 | 零容量改为非阻塞维护 tick；C 允许空事件指针并仍调用运行时维护路径。 |
| 4 | 中 | 明文非法 envelope | 未验证完整性的坏包虽不关闭会话，仍逐包上抛 `ProtocolViolation`，可向游戏循环放大攻击流量。 | 明文坏包静默丢弃并增加无 session 标签的 Prometheus 计数；Noise 验证过的坏包仍 fail-closed。 |
