...
- KCP 旧崩溃样本已回归；无 sanitizer 的覆盖引导 fuzz 已覆盖全部六个 target，并将新发现的恶意 ACK 样本固化为回归。当前仅安装稳定版 Rust 且无 rustup，无法启用 cargo-fuzz 所需的 nightly `-Zsanitizer`；ASan fuzz 与目标环境长稳压测仍未完成。
