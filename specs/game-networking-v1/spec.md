...
- 已实现 TCP/UDP/KCP 共用的有界 `LatestOnly` 游戏层待发队列：按 session+key 合并、轮转背压键、关闭清理、分原因计数，`poll` 自动非阻塞冲刷。更深的传输队列/KCP 缓存撤回不支持；版本范围协商与服务端签名/认证的“选定版本”回复、恢复票据、游戏层 C/C++11/Go API 尚未完成。未实现的其他游戏控制 kind 目前按协议错误拒绝，而不是静默忽略。
