...
- 已实现经 Noise 保护的自动心跳、匹配挑战与超时关闭，以及每会话 RTT/平滑 RTT/抖动快照；心跳依赖应用持续 `poll`，测得延迟包含本地待发送时间。wire v2 的 UDP 业务报文已有库维护序号、有限乱序窗口、可调质量等级快照和两次采样抑制的质量变化事件。KCP 已统计 PUSH 重传与最近 128 段的重传比例，但时钟同步尚未完成。
...
- 已实现 TCP/UDP/KCP 共用的有界 `LatestOnly` 游戏层待发队列：按 session+key 合并、轮转背压键、关闭清理、分原因计数，`poll` 自动非阻塞冲刷。单实例恢复票据、业务重授权、旧→新句柄事件及恢复计数器已落地；跨实例/滚动重启恢复尚未支持。更深的传输队列/KCP 缓存撤回、版本范围协商与服务端签名/认证的“选定版本”回复、游戏层 C/C++11/Go 高级能力对齐尚未完成。未实现的其他游戏控制 kind（包括 `ClockSync`）目前按协议错误拒绝，而不是静默忽略。
- 密钥、鉴权事件、加入配置和恢复身份/票据的默认 `Debug` 输出已脱敏；游戏层在消费鉴权与控制事件后擦除原始事件缓冲，恢复注册表在释放身份副本时擦除其内容。调用方主动提取的业务凭据仍由调用方负责保密。
- Rust 游戏门面已有可选的异步有界结构化诊断日志，覆盖加入/恢复/关闭/安全/质量状态，支持 logger 自身异常计数和 Prometheus；高频业务消息不逐条记录。新增追加式游戏 C ABI 与 C++11/Go 基础门面，覆盖监听、连接、授权、普通发送、`LatestOnly`、轮询、服务端加密切换、单实例票据恢复、每会话质量快照、无玩家标签的 Prometheus 文本及可选 V5 网络容量调优；细粒度指标结构和日志配置尚未完成跨语言对齐。
...
- [ ] 5. 实现可靠 FIFO、`LatestOnly` 替换、优先级公平调度、字节预算与分原因丢弃指标。
  ...
  - [ ] 跨优先级调度、传输队列内尚未写入 socket 的消息撤回与负载压测。
...
- [ ] 8. 将游戏维度接入结构化日志、百分位延迟、Prometheus 文本和关闭原因。
  ...
  - [ ] 实时淘汰聚合诊断与 C/C++11/Go 日志接口仍待补齐。
- [ ] 9. 新增追加式游戏 C ABI，并同步 C++11 与 Go 游戏 facade；普通路径保持参数最少。
  - [x] 普通监听/连接/授权/发送/轮询、服务端加密切换和零 `msg_type`/`stream_id` 的 C/C++11/Go 基础门面及端到端测试。
  - [ ] 细粒度指标结构与日志配置跨语言对齐；保留现有 ABI 版本常量和旧结构布局。
