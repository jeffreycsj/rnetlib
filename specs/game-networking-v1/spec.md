...
- Rust 游戏门面已有可选的异步有界结构化诊断日志，覆盖加入/恢复/关闭/安全/质量状态，支持 logger 自身异常计数和 Prometheus；高频业务消息不逐条记录。追加式游戏 C ABI 与 C++11/Go 门面已覆盖精确/范围版本监听连接、授权、普通发送、`LatestOnly`、轮询、关闭、rekey、服务端加密切换、单实例票据恢复、时钟/质量/队列快照、无玩家标签的 Prometheus 文本及可选 V5 网络容量调优；累计游戏指标和有界结构化日志回调已跨语言对齐。优先级/correlation ID 和按 endpoint/玩家切分的遥测仍未实现。
