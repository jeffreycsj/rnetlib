...
- 已实现 TCP/UDP/KCP 共用的有界 `LatestOnly` 游戏层待发队列：按 session+key 合并、轮转背压键、关闭清理、分原因计数，`poll` 自动非阻塞冲刷。单实例恢复票据、业务重授权、旧→新句柄事件及恢复计数器已落地；跨实例/滚动重启恢复尚未支持。更深的传输队列/KCP 缓存撤回、版本范围协商与服务端签名/认证的“选定版本”回复、游戏层 C/C++11/Go 高级能力对齐尚未完成。未实现的其他游戏控制 kind 仍按协议错误拒绝，而不是静默忽略。
