...
- 已实现 TCP/UDP/KCP 共用的有界 `LatestOnly` 游戏层待发队列：按 session+key 合并、轮转背压键、关闭清理、分原因计数，`poll` 自动非阻塞冲刷。单实例恢复票据、业务重授权、旧→新句柄事件及恢复计数器已落地；跨实例/滚动重启恢复尚未支持。更深的传输队列/KCP 缓存撤回、版本范围协商与服务端签名/认证的“选定版本”回复、游戏层 C/C++11/Go API 尚未完成。未实现的其他游戏控制 kind 目前按协议错误拒绝，而不是静默忽略。
- 密钥、鉴权事件、加入配置和恢复身份/票据的默认 `Debug` 输出已脱敏；游戏层在消费鉴权与控制事件后擦除原始事件缓冲，恢复注册表在释放身份副本时擦除其内容。调用方主动提取的业务凭据仍由调用方负责保密。
- Rust 游戏门面已有可选的异步有界结构化诊断日志，覆盖加入/恢复/关闭/安全/质量状态，支持 logger 自身异常计数和 Prometheus；高频业务消息不逐条记录。跨语言游戏层日志入口仍待实现。
